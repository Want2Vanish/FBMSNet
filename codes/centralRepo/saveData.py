...
def parseBci42aFile(dataPath, labelPath, epochWindow = [0,4], chans = list(range(22))):
    ...
    """FBCSP"""
    #   可能是因为MNE的版本问题   ValueError: Could not find any of the events you specified.
    # raw_gdf = mne.io.read_raw_gdf(dataPath, preload=False, stim_channel="auto")
    # custom_mapping = {'768': 0}
    # (events_from_annot, event_dict) = mne.events_from_annotations(raw_gdf, event_id=custom_mapping)
    # # raw_data.info['bads'] =  raw_data.ch_names[61:]
    # # raw_data.info['bads'] = ['EOG-left', 'EOG-central', 'EOG-right']
    # # picks = mne.pick_types(raw_data.info, meg=False, eeg=True, exclude='bads')
    # epochs = mne.Epochs(raw_gdf, events_from_annot, event_dict, tmin=2, tmax=6, event_repeated='drop',
    ...
    # epochs = epochs.filter(4, 40).resample(raw_gdf.info.get('sfreq'))
